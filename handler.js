...
// NOTE: uncaughtException / unhandledRejection are registered in index.js only.
// Duplicate handlers here caused double-logging and interfered with the
// watchdog's ability to classify crashes correctly.
...
const { scanImageBuffer, scanStickerBuffer } = require('./utils/nsfwScanner');
const { isAutoStatusEnabled, isAutoStatusReactionEnabled } = require('./commands/owner/autostatus');
const fs   = require('fs');
...
setInterval(() => {
  try {
    ...
    // Ban state is managed by database.js in-memory mirror — nothing to invalidate here
  } catch (_) {}
}, 5 * 60 * 1000).unref();
...
const { handleStrictGuard }  = require('./utils/strictGuard');
...
let _mafiaModule = null;
const getMafia = () => {
  try {
    if (!_mafiaModule) _mafiaModule = require('./utils/mafiaGame');
    return _mafiaModule;
  } catch { return null; }
};

// ── Banned users — delegate entirely to database.js in-memory state ───────────
// database.banUser / unbanUser update the in-memory mirror synchronously so
// isBanned() is accurate the instant a ban/unban command runs — no cache lag.
const getBannedUsers = () => database.getBanned();
const _isSenderBanned = (senderJid) => {
  if (!senderJid) return false;
  try { return database.isBanned(senderJid); } catch { return false; }
};
...
// ── Persistent Owner LID cache ────────────────────────────────────────────────
// Stores LID user-parts (e.g. "4027") confirmed to belong to the owner so we
// can recognize them even before the Baileys LID-mapping file is written.
// Loaded at startup, updated the first time a new LID resolves to the owner.
const _OWNER_LIDS_PATH = path.join(__dirname, 'data/ownerLids.json');
const _ownerKnownLids  = (() => {
  try {
    if (fs.existsSync(_OWNER_LIDS_PATH))
      return new Set(JSON.parse(fs.readFileSync(_OWNER_LIDS_PATH, 'utf8')));
  } catch (_) {}
  return new Set();
})();
function _persistOwnerLid(lidUser) {
  if (!lidUser || _ownerKnownLids.has(lidUser)) return;
  _ownerKnownLids.add(lidUser);
  try {
    const dir = path.dirname(_OWNER_LIDS_PATH);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(_OWNER_LIDS_PATH, JSON.stringify([..._ownerKnownLids], null, 2));
    console.log(`[OwnerLID] Saved new owner LID: ${lidUser}`);
  } catch (_) {}
}
    ...
    try {
      await sock.sendMessage(from, {
        text:
          ...
          `This content is not allowed in this group.\n\n` +
          `👤 User: @${sender.split('@')[0]}\n` +
          `📌 Reason: ${reason}\n` +
    ...
    try {
      await sock.sendMessage(from, {
        text:
          ...
          `👤 User: @${sender.split('@')[0]}\n` +
          `📌 Reason: ${reason}\n` +
          `⚠️ Message deleted. (Give bot admin rights to also remove the user.)`,
        mentions: [sender]
      });
    } catch (_) {}
  }
...
const handleMessage = async (sock, msg) => {
  try {
    // ── HARD-LOCKED owner check — 4-layer resolution, always finds 923329838699 ─
    const isOwner = (sender) => {
      if (!sender) return false;

      // Layer 1 ─ Raw phone-number JID  (923329838699@s.whatsapp.net or :device suffix)
      const rawNum = sender.split('@')[0].split(':')[0].replace(/[^0-9]/g, '');
      if (rawNum === OWNER_NUMBER) return true;

      // Layer 2 ─ Persistent LID cache  (survives restarts; populated on first resolution)
      const lidUser = sender.split('@')[0].split(':')[0];
      if (_ownerKnownLids.has(lidUser)) return true;

      // Layer 3 ─ buildComparableIds  (resolves both LID→phone and phone→LID)
      try {
        const variants = buildComparableIds(sender);
        for (const v of variants) {
          const n = (v || '').split('@')[0].split(':')[0].replace(/[^0-9]/g, '');
          if (n === OWNER_NUMBER) {
            if (sender.includes('@lid') || sender.includes('@hosted.lid'))
              _persistOwnerLid(lidUser);   // cache for future lookups
            return true;
          }
        }
      } catch (_) {}

      // Layer 4 ─ normalizeJidWithLid direct (belt-and-suspenders)
      try {
        const resolved    = normalizeJidWithLid(sender) || '';
        const resolvedNum = resolved.split('@')[0].split(':')[0].replace(/[^0-9]/g, '');
        if (resolvedNum === OWNER_NUMBER) {
          if (sender.includes('@lid') || sender.includes('@hosted.lid'))
            _persistOwnerLid(lidUser);
          return true;
        }
      } catch (_) {}
    ...
    // ── Auto-status: view + react + log (toggled via .autostatus on/off) ────
    if (from === 'status@broadcast') {
      const statusSender = msg.key.participant || msg.key.remoteJid;

      // Only process if auto-status is enabled by owner command
      if (isAutoStatusEnabled()) {
        ...
        // React with 💚 only if reactions are also enabled
        if (isAutoStatusReactionEnabled()) {
          try {
    ...
    // ── Auto-populate owner LID cache on very first LID-format message ────────
    // When a group uses Account-ID (LID) addressing and the LID→phone mapping
    // file already exists, we resolve it and save the LID so every subsequent
    // message skips the file-system lookup entirely.
    if (!msg.key.fromMe && sender &&
        (sender.includes('@lid') || sender.includes('@hosted.lid'))) {
      try {
        const _lu = sender.split('@')[0].split(':')[0];
        if (!_ownerKnownLids.has(_lu)) {
          const _vars = buildComparableIds(sender);
          if (_vars.some(v =>
              (v || '').split('@')[0].split(':')[0].replace(/[^0-9]/g, '') === OWNER_NUMBER
          )) {
            _persistOwnerLid(_lu);
          }
        }
      } catch (_) {}
    }

    // ── GLOBAL BAN GATE — runs before EVERY feature (commands, AI, emoji, salam) ──
    // Banned users are silently dropped here. Owner is always exempt.
    if (!msg.key.fromMe && sender && !isOwner(sender) && _isSenderBanned(sender)) return;
    ...
    // ── NSFW SHIELD — per-group toggled (gs.nsfw must be true) ──────────────
    ...
    try {
      if (isGroup && groupMetadata && !msg.key.fromMe) {
        const _nsfwGs = database.getGroupSettings(from);
        if (!_nsfwGs.nsfw) { /* NSFW detection OFF for this group — skip */ }
        else {
        ...
        if (!senderIsOwner) {
          ...
          if (!senderIsAdmin) {
            ...
            // ── Sticker: tag check + actual pixel scan ────────────────────────
            if (mediaKey === 'stickerMessage') {
              ...
              try {
                const stkBuf = await downloadMediaMessage(
                  msg, 'buffer', {}, { reuploadRequest: sock.updateMediaMessage }
                );
                const { nsfw, reason } = await scanStickerBuffer(stkBuf);
                if (nsfw) {
                  await enforceNsfw(sock, msg, from, sender, groupMetadata, `🔞 Adult sticker pixels (${reason})`);
                  return;
                }
              } catch (_scanErr) { /* fail-open */ }
            }
            ...
            if (hasBotSig || forwardScore > 50 || isBehav) {
              ...
              try {
                await sock.sendMessage(from, {
                  text:
                    `*[ 𝐌𝐫.𝐌𝐮𝐧𝐞𝐞𝐛 𝐀𝐥𝐢 𝐁𝐨𝐭 ]*\n\n` +
                    `🚫 *Unauthorized Bot/User Detected!*\n` +
    ...
    // ── Strict Guard — message protection layer ───────────────────────────────
    // Runs only when .strictguard is ON for this group.
    // Checks: flood, repeat spam, mass-mention, length-bomb.
    // Admins and bot owner are always bypassed.
    try {
      if (isGroup && groupMetadata && !msg.key.fromMe) {
        const { checkStrictGuardMessage } = require('./utils/strictGuard');
        const senderIsAdmin = await isAdmin(sock, sender, from, groupMetadata);
        const botIsAdmin    = await isBotAdmin(sock, from, groupMetadata);
        const sgResult = await checkStrictGuardMessage(sock, from, msg, sender, {
          isAdmin   : senderIsAdmin,
          isOwner   : isOwner(sender),
          isBotAdmin: botIsAdmin,
          body,
        });
        if (sgResult.blocked) return;
      }
    } catch (_) {}

    // ── Competitor-bot / hidden-bot shield (ALL message types) ───────────────
    // CRITICAL: must run for EVERY message type (button, list, template,
    // interactive, sticker, image, video, etc) — silent bots without text
    // were previously bypassing detection because we filtered to text-only.
    if (isGroup && groupMetadata && !msg.key.fromMe) {
      try {
        const gs = database.getGroupSettings(from);
        // Pass full settings + role info so antiBot.js can decide
        handleCompetitorBot(sock, msg, groupMetadata, {
          ...gs,
          sender, from, senderIsOwner: isOwner(sender)
        }).catch(() => {});
      } catch (_) {}
    }

    // ── External-promotion / channel-link guard (always on in groups) ────────
    // Detects channel invites, group invites, t.me, wa.me from non-admin
    // non-owner. Action: delete + warn → remove on repeat.
    if (isGroup && groupMetadata && !msg.key.fromMe) {
      try {
        const rc = msg.message?.ephemeralMessage?.message || msg.message || {};
        const linkText = [
          rc?.conversation, rc?.extendedTextMessage?.text,
          rc?.imageMessage?.caption, rc?.videoMessage?.caption,
          rc?.documentMessage?.caption,
          // newsletter/channel-share payloads can carry the link inline
          rc?.extendedTextMessage?.contextInfo?.externalAdReply?.sourceUrl,
          rc?.groupInviteMessage?.groupJid,
          rc?.newsletterAdminInviteMessage?.newsletterJid
        ].filter(Boolean).join(' ');

        const PROMO_PATTERNS = [
          /chat\.whatsapp\.com\/[A-Za-z0-9]{10,}/i,           // group invite
          /whatsapp\.com\/channel\/[A-Za-z0-9]+/i,            // channel link
          /wa\.me\/(?!923329838699\b)\d{5,}/i,                // wa.me (allow owner)
          /t\.me\/[A-Za-z0-9_]+/i,                            // telegram
          /(?:https?:\/\/)?discord(?:app)?\.(?:gg|com\/invite)\/\S+/i,
          /\bjoin\s+(?:my|our|this)\s+(?:channel|group|whatsapp)\b/i
        ];
        const hasPromo = !!rc?.groupInviteMessage
                      || !!rc?.newsletterAdminInviteMessage
                      || PROMO_PATTERNS.some(re => re.test(linkText));
        ...
        if (hasPromo) {
          ...
          if (!senderIsAdmin && !isOwner(sender)) {
            ...
            if (!global.__promoOffenders) global.__promoOffenders = new Map();
            const key = `${from}|${sender}`;
            const count = (global.__promoOffenders.get(key) || 0) + 1;
            global.__promoOffenders.set(key, count);
            if (botIsAdmin && count >= 2) {
              try { await sock.groupParticipantsUpdate(from, [sender], 'remove'); } catch (_) {}
              try { await sock.sendMessage(from, {
                text: `🚫 *External Promotion Blocked*\n\n@${sender.split('@')[0]} removed for promoting external channels/links.`,
                mentions: [sender]
              }); } catch (_) {}
              global.__promoOffenders.delete(key);
            } else {
              try { await sock.sendMessage(from, {
                text: `⚠️ @${sender.split('@')[0]} external links/channel promotions are not allowed here.`,
                mentions: [sender]
              }); } catch (_) {}
            }
            return;
      ...
      } catch (e) { console.error('[PromoGuard]', e.message); }
    }
    ...
    // ── Owner-mention reply (strict mode) ─────────────────────────────────────
    // FIRES ONLY WHEN:
    //   • Message text contains the literal owner number "923329838699"
    //     (with or without leading "+"), AND
    //   • Sender is not the bot, AND
    //   • Message is NOT a reply / quote, AND
    //   • Message is NOT a forward, AND
    //   • We haven't already replied to this exact message ID.
    // No JID/session matching — pure text match against the owner number only.
    if (!msg.key.fromMe) {
      try {
        const OWNER_DIGITS = '923329838699'; // strict: this number only
        const text = String(body || '');

        // Trigger A: digits literally in the text ("923329838699" or "+923329838699")
        const containsOwnerInText = text.includes(OWNER_DIGITS);

        // Trigger B: owner JID present in mentionedJid (covers @-tag of saved contact
        // where the visible text is "@SomeName" but the digits live in the JID array).
        // Expanded to cover ALL message types that can carry mentions.
        const _mc2 = msg.message?.ephemeralMessage?.message
                  || msg.message?.viewOnceMessageV2?.message
                  || msg.message?.viewOnceMessage?.message
                  || msg.message
                  || {};
        const ctx =
          _mc2?.extendedTextMessage?.contextInfo   ||
          _mc2?.imageMessage?.contextInfo          ||
          _mc2?.videoMessage?.contextInfo          ||
          _mc2?.documentMessage?.contextInfo       ||
          _mc2?.audioMessage?.contextInfo          ||
          _mc2?.stickerMessage?.contextInfo        ||
          content?.extendedTextMessage?.contextInfo ||
          {};
        const mentioned = Array.isArray(ctx.mentionedJid) ? ctx.mentionedJid : [];
        const containsOwnerInMentions = mentioned.some(j =>
          String(j || '').split('@')[0].split(':')[0] === OWNER_DIGITS
        );

        // Trigger C: text literally contains "@923329838699" (typed mention without saving contact)
        const containsOwnerAtMention = text.includes(`@${OWNER_DIGITS}`);

        const containsOwner = containsOwnerInText || containsOwnerInMentions || containsOwnerAtMention;

        if (containsOwner) {
          const isReply     = !!(ctx.stanzaId || ctx.quotedMessage);
          const isForwarded = !!ctx.isForwarded || (Number(ctx.forwardingScore) || 0) > 0;

          if (!isReply && !isForwarded) {
            // Per-message dedupe (in-memory LRU, capped at 500 entries to avoid leaks)
            if (!global.__ownerMentionSeen) global.__ownerMentionSeen = new Set();
            const seen = global.__ownerMentionSeen;
            const msgId = msg.key.id;
            if (msgId && !seen.has(msgId)) {
              seen.add(msgId);
              if (seen.size > 500) {
                // Drop oldest ~100 entries when cache grows
                const it = seen.values();
                for (let i = 0; i < 100; i++) seen.delete(it.next().value);
              }
              console.log(`[OWNER-MENTION] triggered for owner ${OWNER_DIGITS} in ${from} (msg ${msgId})`);
              const replies = [
                '𝗕𝗢𝗦𝗦 𝗕𝗨𝗦𝗬 𝗛𝗔𝗜𝗡🙁',
                '𝗠𝗥. 𝗠𝗨𝗡𝗘𝗘𝗕 𝗔𝗟𝗜 𝗔𝗕𝗛𝗜 𝗢𝗙𝗙𝗟𝗜𝗡𝗘 𝗛𝗔𝗜𝗡⚡',
                '𝗕𝗢𝗦𝗦 𝗜𝗦 𝗡𝗢𝗧 𝗛𝗘𝗥𝗘 𝗥𝗜𝗚𝗛𝗧 𝗡𝗢𝗪👑',
                '𝗕𝗢𝗦𝗦 𝗞𝗔𝗛𝗜𝗡 𝗕𝗨𝗦𝗬 𝗛𝗔𝗜𝗡⚠️'
              ];
              await sock.sendMessage(from, {
        ...
        console.error('[OWNER-MENTION] error:', e?.message || e);
      }
    }
    ...
    // ── Mafia game: DM night-action intercept (.kill / .save / .check) ────────
    // Group-side actions (.join / .leave / .vote) are handled by their command files.
    // Only runs when sender is a player and is messaging the bot in DM.
    try {
      const mafia = getMafia();
      if (mafia?.handleMafiaDmAction && !isGroup) {
        const normSender = sender.replace(/:[0-9]+@/, '@');
        if (mafia.playerToGame?.has(normSender)) {
          const handled = await mafia.handleMafiaDmAction(sock, msg, sender, from, body);
          if (handled) return;
        }
      }
    } catch (_) {}
    ...
    if (!body.startsWith(config.prefix)) {
      ...
      // ── Chatbot state — read once, gates ALL auto-response features below ──
      // For groups: features only fire when chatbot is explicitly ON.
      // For private chats: always allow (chatbot default is ON in private).
      let _chatbotActive = true;
      if (isGroup) {
        try {
          const _cbPath = path.join(__dirname, 'data/chatbotGroups.json');
          const _cbCfg  = fs.existsSync(_cbPath)
            ? JSON.parse(fs.readFileSync(_cbPath, 'utf8'))
            : {};
          _chatbotActive = _cbCfg[from] === true;
        } catch (_) { _chatbotActive = false; }
      }
      // Owner always passes through regardless of chatbot state
      const _senderIsOwner = isOwner(sender);

      // ── Reply-only gate ───────────────────────────────────────────────────
      // ALL auto-response features (emoji, salam, hi, AI chatbot) only fire
      // when the incoming message is a REPLY to another message.
      // Plain messages → bot stays completely silent. No spam, no random replies.
      const _replyCtx =
        ...
        null;
      const _isReply = !!(_replyCtx?.quotedMessage);

      if (!msg.key.fromMe) {
        // Emoji map — only fires on reply + chatbot ON (or private chat, or owner)
        if (_isReply && (_chatbotActive || !isGroup || _senderIsOwner)) {
          try {
            const emojiMap = {
      ...
      // ── Auto Salam reply — reply-only + chatbot state gated ─────────────
      if (_isReply && (_chatbotActive || !isGroup || _senderIsOwner)) {
        try {
          const { handleSalam } = require('./utils/salamReply');
          const salamHandled = await handleSalam(sock, from, msg, body, sender);
          if (salamHandled) return;
        } catch (_) {}
      }

      // ── Hi / Hello polite correction — reply-only + chatbot state gated ─
      if (_isReply && (_chatbotActive || !isGroup || _senderIsOwner)) {
        try {
          const { handleHiCorrection } = require('./utils/hiCorrection');
          const hiHandled = await handleHiCorrection(sock, from, msg, body, sender);
          if (hiHandled) return;
        } catch (_) {}
      }
      ...
      try {
        const { handleAIChat } = require('./utils/aiChat');
        await handleAIChat(sock, from, msg, body, sender, {
          isGroup,
          isOwner : isOwner(sender),
          botJid  : sock.user?.id || '',
        });
    ...
    if (command.adminOnly && !(await isAdmin(sock, sender, from, groupMetadata)) && !isOwner(sender)) {
      console.log(`[AdminCheck] BLOCKED — cmd=${commandName} sender=${sender} isOwner=${isOwner(sender)} rawNum=${sender.split('@')[0].split(':')[0]} OWNER=${OWNER_NUMBER}`);
    ...
    // ── Strict Guard — command flood gate ────────────────────────────────────
    // Runs only when .strictguard is ON for this group.
    // Admins and owner always bypass.
    try {
      if (isGroup) {
        const { checkStrictGuardCommand } = require('./utils/strictGuard');
        const sgCmd = await checkStrictGuardCommand(sock, from, commandName, sender, {
          isAdmin : cmdCtx.isAdmin,
          isOwner : cmdCtx.isOwner,
        });
        if (sgCmd.blocked) return;
      }
    } catch (_) {}
...
const handleGroupUpdate = async (sock, update) => {
  try {
    ...
    // ── Strict Owner Guard — Zero Trust Mode ──────────────────────────────────
    // Must run BEFORE any other logic so corrective actions fire first.
    // Runs only on promote/demote; is a no-op when strictguard is off for the group.
    // All exceptions are caught internally — will never crash handleGroupUpdate.
    if (action === 'promote' || action === 'demote') {
      handleStrictGuard(sock, update).catch(() => {});
    }
